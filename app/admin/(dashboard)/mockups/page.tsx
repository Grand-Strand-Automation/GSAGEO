...
export default async function AdminMockupsPage() {
  ...
  const { data, error } = await supabase
    ...
    .select(
      "id, created_at, website_url, business_name, business_category, preferred_style, homepage_goal, email, status, result_viewed_at, subscription_cta_clicked_at, selected_plan_intent, notes",
    )
    ...
    <div className="container px-4 md:px-6 py-8 max-w-6xl">
      ...
      {error && (
        <div className="card-brand p-6 mb-6 text-sm text-brand-muted">
          Could not load mockup leads. If this is a new environment, apply migration{" "}
