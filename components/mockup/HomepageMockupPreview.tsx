...
export function HomepageMockupPreview({ concept }: { concept: MockupConcept }) {
  const { theme } = concept;
  ...
  return (
    <div className="rounded-2xl border border-brand-border bg-brand-cream/40 p-3 md:p-4 shadow-card-md">
      ...
      >
        ...
        >
          <div className="font-heading font-extrabold text-sm md:text-base tracking-tight">
            {concept.businessName}
          </div>
          <nav className="hidden sm:flex items-center gap-5 text-xs font-medium" style={{ color: theme.muted }}>
            {concept.navItems.map((item) => (
              <span key={item}>{item}</span>
            ))}
          </nav>
          <span
            className="text-[11px] font-bold uppercase tracking-wide px-3 py-1.5 rounded-md"
        ...
        >
          ...
          <h3 className="font-heading font-extrabold text-2xl md:text-4xl leading-[1.1] max-w-2xl mb-4">
            {concept.headline}
          </h3>
          ...
          <div className="flex flex-wrap gap-3">
            <span
              ...
              style={{ background: theme.accent === "#c45c26" ? theme.accent : "#fff", color: theme.accent === "#c45c26" ? "#fff" : theme.text }}
            >
              {concept.primaryCta}
            </span>
        ...
        <footer
          ...
          style={{ color: theme.muted, borderColor: `${theme.muted}18`, background: theme.surface }}
        >
          {concept.footerNote}
        </footer>
      </div>
